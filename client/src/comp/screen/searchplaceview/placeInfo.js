...
const PlaceInfoView = (props) => {
  ...
  useEffect(() => {
    if (!lodash.isNil(selectedPlaceCoordinate)) {
      ...
      fetchLocationInfo(selectedPlaceCoordinate);
    }
  }, [selectedPlaceCoordinate]);

  const fetchLocationInfo = async (coords) => {
    try {
      updateState({ isLoadingLocation: true });
  ...
  const isSearchPlaceSectionWithinSettings = (type) => {
    ...
    let searchPlaceSectionArray = appSettingObj?.searchPlaceSectionArray ?? [
      ...
      <TableContainer mt={2} overflowX={"auto"}>
        <Table size="sm">
          ...
          <Tbody>
            <Tr>
              <Td textAlign="left">
                <Tooltip hasArrow placement="top" label={"Decimal Degrees"}>
                  DD
                </Tooltip>
              </Td>
              <Td
                    ...
                    (favPlaceObj, index) => {
                      return (
                        <Box paddingY={1} key={`favPlace-${index}`}>
                          ...
                          {index <
                            (state?.favPlaceDisplayArray ?? []).length - 1 && (
                            <Divider pt={2} mb={2} />
                          )}
                        </Box>
                      );
